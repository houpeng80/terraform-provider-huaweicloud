...
func main() {
	...
	errCount := checkApiComments(basePath)
	os.Exit(errCount)
}
...
func apiCommentCheck(commentStr string) int {
	...
	if strings.HasPrefix(commentStr, "// API") && !strings.HasPrefix(commentStr, "// API:") {
		fmt.Printf("[ERROR] the format of `%s` is not correct, should start with (%s)\n", commentStr, commentPrefix)
		return 1
	}
	...
	if !strings.HasPrefix(url, "/") {
		fmt.Printf("[ERROR] the request url of `%s` is not correct, should start with /\n", commentStr)
		return 1
	}
	if strings.HasSuffix(url, "/") {
		fmt.Printf("[ERROR] the request url of `%s` is not correct, should not end with /\n", commentStr)
		return 1
	}
	return 0
}
